...
Loss=nn.MSELoss()
a=[10,19,35,52,61,80,89,105,122,131,150,159,175,192,201]
train_features=torch.transpose(torch.load(f'train_features.pth'),dim0=1,dim1=2)
train_labels=torch.load(f'train_labels.pth')
test_features=torch.transpose(torch.load(f'test_features.pth'),dim0=1,dim1=2)
test_labels=torch.load(f'test_labels.pth')
...
for i in a:
    print(i)
    train_features=torch.transpose(torch.load(f'case{i}_x.pth'),dim0=1,dim1=2)
    train_labels=torch.load(f'case{i}_labels.pth')
    train_iter=d2l.load_array((train_features,train_labels),batch_size,is_train=True)
    train_loss=evaluate_loss(net,train_iter,Loss)
    print(train_loss)
